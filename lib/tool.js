...
// sniffing (never trusts extensions) and a size pre-check before reading bytes.
// The 0.4.x LRU parse cache was removed in 0.5.0 — measured parse cost
// (180-235 ms for a 1.1 MB PDF) is noise next to model latency, so the cache
// could not prove its necessity in the 0.5.0 ablation.
import { defineTool } from '@deepseek-ai/dsh-tools';
import { FsError } from '@deepseek-ai/dsh-fs';
import { sniffFormat, sniffHead, HEAD_SNIFF_BYTES, SUPPORTED_FORMATS, formatFromExtension } from "./detect.js";
import { runParser } from "./parse/runner.js";
import { parseLimits } from "./parse/limits.js";
/**
...
export function formatOutputBudget(format, base) {
    ...
    if (format === 'pdf' || format === 'docx')
        return Math.min(base, Math.max(2000, Math.floor(base / 2)));
    // xlsx：结构化表格信息密度高，但行多列宽也会撑爆上下文；给 3/4，配合
    // windowLines 的截断标记引导模型 offset 翻页增量取。
    if (format === 'xlsx')
        return Math.min(base, Math.max(2000, Math.floor(base * 0.75)));
    return base;
}
function parseArgs(args, config) {
    ...
    if (args.list_sheets !== undefined && typeof args.list_sheets !== 'boolean')
        throw new Error('list_sheets must be a boolean');
...
export function defineReadDocumentTool(ctx, config) {
    ...
    return defineTool({
        ...
        async execute(args, exec) {
            ...
            if (activeReads >= limits.maxConcurrentReads)
                throw new Error('document reader busy: concurrent read budget reached; retry after pending reads finish');
            activeReads++;
            try {
