 ...
 * blank line.
 */
export async function parsePdf(bytes, limits = DEFAULT_PARSE_LIMITS) {
    ...
    try {
        if (doc.numPages > limits.maxPdfPages)
            throw new Error('PDF exceeds page budget');
        const pages = [];
        let chars = 0;
        for (let pageNo = 1; pageNo <= doc.numPages; pageNo++) {
            ...
            try {
                ...
                if (chars > limits.maxParsedChars)
                    throw new Error('PDF exceeds parsed text budget');
                pages.push(pageText);
            }
            finally {
                page.cleanup();
